...
export default function Home(){
 ...
 <button className="hint-button" disabled={phase!=='ready'} onClick={()=>{const best=data?.moves.find(m=>problem&&preservesGoal(m,problem.goal));if(best){setHint(best.uci);setSelected(best.uci.slice(0,2));}}}>{hint?`${hint.slice(0,2)} → ${hint.slice(2,4)}${hint[4]?' = '+names[hint[4]]:''}`:'✧ 最善手のヒント'}</button><button className="hint-button" disabled={phase!=='ready'} onClick={()=>{const best=data?.moves.find(m=>problem&&preservesGoal(m,problem.goal));if(best)void play(best);}}>答え：最善手を指す →</button><p className="verified">{problem?'✓ テーブルベースで検証した局面':'局面を検証しています'}</p></section>
