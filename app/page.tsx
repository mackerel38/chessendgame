...
export default function Home(){
 ...
 function moveInstantly(c:Chess,uci:string,signal:AbortSignal){if(signal.aborted)return;applyUci(c,uci);setAnimation(null);setFen(c.fen());}
 ...
 async function sync(c:Chess,p:Problem,signal:AbortSignal,known?:Tablebase){
  ...
  try{if(endGame(c,p))return;let tb=known||await probe(c.fen(),signal);if(signal.aborted)return;
   if(c.turn()!==p.fen.split(' ')[1]){const best=tb.moves[0];if(!best||outcome(best.category)===null)throw new Error('最善応手を確認できません。');moveInstantly(c,best.uci,signal);if(signal.aborted)return;setHistory(h=>[...h,{san:best.san,player:false,quality:'最善応手'}]);if(endGame(c,p))return;tb=await probe(c.fen(),signal);}
 ...
 async function replay(move:TBMove,p:Problem,tb:Tablebase,answer=false){
  const signal=controller.current.signal;replaying.current=true;beforeReplay.current=tb;locked.current=true;setPhase('replay');setReplayMode(answer?'answer':'mistake');setSelected('');setHint('');setPromotion([]);if(!answer)setMistakes(v=>v+1);setReplayLine([move.san]);setReplayStatus(`${p.goal==='win'?'勝ち':'引き分け'} → ${outcome(move.category)===1?'負け':'引き分け'}`);if(answer)setReplayStatus('答え：終局までの最善進行');setFeedback(answer?'答えをアニメーション再生中':'誤った手からの最善進行を再生中');
  const c=cloneGame(game.current,p.fen);
  try{await animate(c,move.uci,signal);let plies=0;
   while(!c.isGameOver()&&(answer||plies<400)){while(pauseRef.current)await delay(100,signal);await delay(speedRef.current,signal);const next=await probe(c.fen(),signal);if(signal.aborted)return;while(pauseRef.current)await delay(100,signal);const best=next.moves[0];if(!best||outcome(best.category)===null)throw new Error('続きの最善手を確認できませんでした。');await animate(c,best.uci,signal);setReplayLine(line=>[...line,best.san]);plies++;}
   const end=terminal(c,player,p.goal);if(answer&&end){replaying.current=false;game.current=c;setHistory(c.history({verbose:true}).map(m=>({san:m.san,player:m.color===player,quality:'解答'})));setPhase('done');setData(null);setFeedback('解答の再生が終了しました。'+end.text);return;}setReplayStatus(end?.text||'400手分で再生を区切りました（終局前）');await delay(1800,signal);if(!signal.aborted)restoreReplay(end?`${end.text} 元の局面に戻りました。`:'長い手順のため再生を区切り、元の局面に戻しました。');
 ...
 async function play(move:TBMove){const p=current.current,tb=data;if(locked.current||!p||!tb)return;if(outcome(move.category)===null){setFeedback('この手の厳密な評価を確認できません。');return;}locked.current=true;setSelected('');setHint('');setPromotion([]);const trial=cloneGame(game.current,p.fen);applyUci(trial,move.uci);const end=terminal(trial,player,p.goal);
  ...
  const signal=controller.current.signal;setPhase('thinking');setData(null);try{moveInstantly(game.current,move.uci,signal);if(signal.aborted)return;setHistory(h=>[...h,{san:move.san,player:true,quality:'✓'}]);setFeedback(p.goal==='win'?'勝ちを維持':'引き分けを維持');await sync(game.current,p,signal);}catch(e){report(e,signal);}
 }
 ...
 <div className="player-line"><span className="avatar white-avatar">♚</span><div><strong>あなた / {player==='w'?'白':'黒'}</strong><small>{phase==='replay'?(replayMode==='answer'?'答えの最善進行を再生中':'失敗手の最善進行を再生中'):phase==='ready'?'駒をドラッグ、またはクリックして移動':phase==='generating'?`局面を生成中 · ${attempt} 局面を照合` :phase==='done'?'練習終了':phase==='error'?'接続待ち':'最善応手を確認中'}</small></div><span className="your-turn">● {phase==='ready'?'YOUR TURN':phase==='replay'?'REPLAY':phase==='done'?'FINISHED':'WAIT'}</span></div>
 ...
 {help&&<div className="modal-backdrop" onClick={()=>setHelp(false)}><section className="modal" role="dialog" aria-modal="true" aria-label="操作ガイド" onClick={e=>e.stopPropagation()}><h2>操作ガイド</h2><ul className="help-list"><li>左ドラッグ／クリック2回：駒を移動</li><li>答え：現在の局面から終局まで最善進行をアニメーション再生</li><li>右ドラッグ：矢印を追加・削除</li><li>右クリック：丸いマークを追加・削除</li><li>左操作／「注釈を消す」：注釈をクリア</li><li>間違えた手：最善進行を再生し、終局後に元へ戻る</li></ul><p className="muted">再生は一時停止・速度変更・打ち切りができます。失敗手の再生は400 plyで区切り、答えは終局まで再生します。局面生成と評価にはインターネット接続が必要です。</p><button className="primary" onClick={()=>setHelp(false)}>閉じる</button></section></div>}
 </main>;
}
